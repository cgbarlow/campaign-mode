# SPEC-ADR-C: ADR Dependencies and Relationships

| Field | Value |
|-------|-------|
| **Specification ID** | SPEC-ADR-C |
...
When an ADR status changes, the following process should be followed:
...
Based on ADR Dependencies and Relationships from Recording Architecture Decisions (Expanded).
